...
\textbf{策略概述}：从身份与权限、策略执行、审计与检测、网络收敛以及秘密治理等层面协同落地。核心方法包括对 CSI 相关 RBAC 进行最小化与清理，按需评估 ServiceAccount 硬化措施，在准入侧阻断再次引入含 \texttt{secrets} 读取动词的 ClusterRole，并在审计侧对 \texttt{secrets} 的枚举行为建立可观测与告警。必要时可进一步收敛 CSI 工作负载的出站访问面。
