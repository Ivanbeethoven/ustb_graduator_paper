...
综合以上分析，本文方法针对CVE-2023-30512生成的策略能够有效检测并防护该攻击。通过RBAC最小化直接移除了CSI ServiceAccount不应具备的\texttt{secrets}读取能力，配合准入策略阻断了过度授权的重新引入，复测实验确认了\texttt{kubectl auth can-i}结果从\texttt{yes}变为\texttt{no}。该方案需要在部署前充分验证CSI插件的PVC/PV生命周期功能不受影响，适用于使用第三方CSI插件的Kubernetes集群环境。
