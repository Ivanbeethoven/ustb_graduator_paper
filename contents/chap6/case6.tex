...
\section{案例六：CVE-2022-29171 远程代码执行}
...
将漏洞场景描述输入 KPEDefense 系统，调用 GPT5、GLM-4.5 与 DeepSeek-V3.2 三个大模型生成候选安全策略，依据有效性（E）、干扰性（I）、可部署性（D）三个维度静态评估后筛选最优策略集合。最终选定 GPT5 生成的策略，以切断命令执行入口与隔离执行面为核心，包括禁用或移除 Gitolite 集成、对 \texttt{gitserver} 实施网络隔离，以及收紧外部服务配置与 Grafana 管理权限。详细配置见附录\ref{app:cve-2022-29171}。
