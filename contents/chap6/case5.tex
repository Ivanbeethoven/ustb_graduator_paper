...
# 路径穿越審读文件需要写入 /tmp，用 emptyDir 替代根目录写入
...
spec:
  template:
    spec:
      ...
      - name: manager
        securityContext:
          readOnlyRootFilesystem: true       # 封锁根文件系统，防止一次性写入文件
          runAsNonRoot: true                 # 以非 root 身份运行容器
          allowPrivilegeEscalation: false    # 禸止容器内部权限提升
          capabilities:
      ...
      - name: tmp
        emptyDir:
          sizeLimit: 64Mi  # 限制临时目录大小，防止磁盘填满
'
...
spec:
  ...
  - to:
    ...
    - protocol: TCP
      port: 53
  # 允许访问 source-controller 拉取制品，不直连外网
  - to:
    - podSelector:
        matchLabels:
    ...
    - protocol: TCP
      port: 9090
  # 允许访问 Kubernetes API Server 来应用资源清单
  - to:
    - ipBlock:
