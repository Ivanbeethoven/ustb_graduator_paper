...
\subsection{策略部署}
本节给出可落地的缓解策略。与仅依赖版本升级不同，本案例强调敏感信息泄漏的治理核心在于读权限面与暴露窗口管理。

\textbf{策略概述}：从身份与权限、审计与告警、资源生命周期治理以及审计型准入检测四个层面协同落地。具体方法包括收敛 \texttt{ClusterProvision} 的读取权限，启用读取行为审计并联动告警，在供应完成后对对象进行清理以缩短暴露窗口，并在发现泄露风险后执行 vCenter 凭据轮换与强化。同时，可用 Gatekeeper 或 Kyverno 以 \texttt{dryrun/Audit} 模式对对象中疑似敏感字段进行检测提示，作为治理与回归监测的补充手段。
...
RBAC 与审计属于平台原生能力，适合在多集群场景中模板化推广。清理与轮换更多体现为运维流程治理，建议与 GitOps 或流水线集成，例如在供应完成后打标签并由定时任务触发清理。准入策略建议从 \texttt{dryrun/Audit} 起步，用于检测与辅助治理，通常不建议强制拦截控制器写入 \texttt{status} 字段，以避免影响供应流程。
