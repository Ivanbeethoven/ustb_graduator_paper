...
针对Kubernetes权限提升漏洞问题，本文提出一种基于多智能体安全策略管理的智能化漏洞防控技术，基于多种安全工具的策略化代码生成、评估与部署实现对权限提升相关漏洞的动态防护与监控,开发了对应的原型系统。本文的主要工作如下。

\textbf{（1）多智能体安全策略管理的漏洞防控技术}：首先,本文收集了近年来Kubernetes第三方云原生应用中的权限提升漏洞样本并设计了漏洞成因分类框架，同时基于开源代码仓库中等多源信息构建了云原生权限提升安全知识库。然后，构建由安全知识、代码生成与安全评审等六个智能体组成的工作流，进行针对特定权限提升漏洞防护与监控策略代码的自动生成、迭代优化与质量评估。最后，针对安全策略代码的实际应用需求，设计了覆盖有效性、非干扰性与可部署性的三维质量评价指标。

\textbf{（2）开发原型系统}：设计并实现原型系统KPEAgent，集成知识库管理、策略生成工作流、专家评审排序与策略部署验证等功能模块，形成面向云原生权限提升漏洞的策略化防控流程。

\textbf{（3）实验验证与案例分析}：在 55 个漏洞样本上，基于 5 种大语言模型开展消融实验与模型对比实验。结果表明，完整工作流在三个评价维度上均取得最优表现，其中反馈优化对策略质量的影响最为显著，移除该模块后策略有效性下降 79\%。与四种静态扫描工具相比，本文方法能够围绕具体漏洞生成更具针对性的临时防控方案。进一步地，本文选取 7 个覆盖不同成因类型的典型 CVE，在隔离集群中完成漏洞复现与策略部署验证，从三个维度进一步验证了所提方法的实际效果。
