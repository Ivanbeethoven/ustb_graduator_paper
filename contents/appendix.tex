...
\subsection{4）"敏感字段守护"检测（Gatekeeper/Kyverno，建议审计模式）}
\begin{AppendixCode}
sensitive_key(k) {
				lk := lower(k)
...
\subsection{2）准入防回归（OPA Gatekeeper）：禁止创建包含 secrets 读取动词的 ClusterRole}
\begin{AppendixCode}
...
\subsection{3）审计与检测：对 \texttt{secrets} 枚举行为进行可观测化}
\begin{AppendixCode}
apiVersion: audit.k8s.io/v1
kind: Policy
rules:
...
\subsection{1）网络隔离：限制 \texttt{gitserver} 的可达面（默认拒绝并按需放通）}
\begin{AppendixCode}
NAMESPACE=sourcegraph
...
\subsection{权限收敛：RBAC 最小化与项目边界约束}
\begin{AppendixCode}
apiVersion: v1
kind: ConfigMap
metadata:
