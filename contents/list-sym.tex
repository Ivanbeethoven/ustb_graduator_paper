...
\chapter*{符号清单与术语表}\addcontentsline{toc}{chapter}{符号清单与术语表}\label{chap:sym-list}

全文中频繁使用的主要符号、核心术语与常用缩写汇总如下。为避免与正文重复，本页重点列出第三章方法定义与后续评价分析中反复出现的记号与表述。
...
$\mathcal{A}$ & 生成方法配置集合，用于刻画提示词组织、目标侧重或工作流设定等差异。 \\
$\mathcal{M}$ & 底层大语言模型集合，表示参与候选策略生成的模型类型。 \\
$\mathcal{G}_v$ & 漏洞 $v$ 的生成配置集合，满足 $\mathcal{G}_v\subseteq\mathcal{A}\times\mathcal{M}$。 \\
$g=(a,h)$ & 生成配置，表示方法配置 $a$ 与底层大语言模型 $h$ 的组合（定义~\ref{def:gen-config}）。 \\
$\Pi_v^{(g)}$ & 漏洞 $v$ 在生成配置 $g$ 下输出的候选安全策略子集。 \\
$\Pi_v$ & 漏洞 $v$ 的候选策略集合，由不同生成配置输出的候选安全策略子集汇合而成。 \\
$\tau$ & 类型变量，表示某一给定策略类型。 \\
$\Pi_v^{\tau}$ & 同类型候选子集，表示漏洞 $v$ 在类型 $\tau$ 下的候选安全策略集合。 \\
$\Pi_v^{\mathrm{mon}},\Pi_v^{\mathrm{prot}}$ & 漏洞 $v$ 的监控型与防护型候选子集，是 $\Pi_v^{\tau}$ 的两种具体情形。 \\[0.4ex]
% ---- 评价与聚合记号 ----
...
$n_{\tau}$ & 同类型候选集合 $\Pi_v^{\tau}$ 的规模，即 $|\Pi_v^{\tau}|$。 \\
...
为统一全文表述，以下对全文使用频率较高的核心术语与常用缩写作简要说明。
\begingroup
\small
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{0.88}
